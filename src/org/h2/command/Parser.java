...
import org.h2.command.dml.Follow;
...
import org.h2.util.TriggerDirectlyFollows2;
...
public class Parser {
    ...
    private Follow parseFollow(){
    	Follow command = new Follow(session);
    ...
    private JavaFunction readJavaFunction(Schema schema, String functionName) {
        ...
        if ((functionAlias == null) && functionName.equalsIgnoreCase("FOLLOWS")){
        ...
        } else if ((functionAlias == null) && functionName.equalsIgnoreCase("DIRECTLYFOLLOWS")) {
        	Database db = session.getDatabase();
        	Schema schemaForFunction = (schema != null)?getSchema():database.getSchema(session.getCurrentSchemaName());
            functionAlias = FunctionAlias.newInstance(schemaForFunction, db.allocateObjectId(),
        	...
        	Schema schemaForFunction = (schema != null)?getSchema():database.getSchema(session.getCurrentSchemaName());
            functionAlias = FunctionAlias.newInstance(schemaForFunction, db.allocateObjectId(),
                    "CONTROLLER", "org.h2.util.ControllerFunction.controllerFunction", false, true);
            functionAlias.setDeterministic(false);
            db.addSchemaObject(session, functionAlias);
        }   else if ((functionAlias == null) && functionName.equalsIgnoreCase("AUTOFOLLOWS")) {
        	System.out.println("find autofollows");
        	Database db = session.getDatabase();
        	Schema schemaForFunction = (schema != null)?getSchema():database.getSchema(session.getCurrentSchemaName());
            functionAlias = FunctionAlias.newInstance(schemaForFunction, db.allocateObjectId(),
        ...
        System.out.println("create java function: " + functionAlias.toString());
        return func;
    }
